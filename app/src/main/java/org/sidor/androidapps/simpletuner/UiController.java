...
public class UiController implements Observer, OnItemSelectedListener {
	...
	private void updateUi() {
		...
		ui.updatePitchView(frequency, current.freq);
		...
		switch(message) {
			case TUNING_IN_PROGRESS:
				ui.displayMessage("Currently tuning string " + current.name +
						", matched in " + Math.round(100.0*match) + "%.", "Current frequency: "
						+ Math.round(frequency*100.0)/100.0 + "\nTarget Frequency: " + current.freq, true);
				break;
			case TOO_NOISY:
				ui.displayMessage("Too much background noise","Current frequency: "
						+ Math.round(frequency*100.0)/100.0 + "\nTarget Frequency: " + current.freq, false);
				break;
			case TOO_QUIET:
				ui.displayMessage("Play louder!","Current frequency: "
						+ Math.round(frequency*100.0)/100.0 + "\nTarget Frequency: " + current.freq, false);
				break;
			case WEIRD_FREQUENCY:
				ui.displayMessage("Frequency out of range","Current frequency: "
						+ Math.round(frequency*100.0)/100.0 + "\nTarget Frequency: " + current.freq, false);
			default:
	...
	public void onItemSelected(AdapterView<?> parent, View thing, int itemno,
		...
		Log.d(TAG,"Passed tuning on select(UICONT)");
	}
